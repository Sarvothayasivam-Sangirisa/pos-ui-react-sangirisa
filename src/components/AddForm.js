...
const AddForm = ({ onClose }) => {
  ...
  {["GST", "HST", "VAT", "Tax"].map((tax) => (
    <label key={tax} className="flex items-center gap-2 text-sm font-medium text-gray-700">
  ...
  <div className="flex items-center space-x-4">
    <div className="space-y-1">
      ...
      <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 bg-gray-50 hover:bg-gray-100 cursor-pointer h-10 w-48 flex items-center space-x-2">
        <HiUpload className="text-gray-400 text-lg" />
      </div>
    </div>

    <div className="space-y-1">
      <label className="block text-sm font-medium text-gray-700">Use Scale</label>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, unitScale: e.target.value })}
                    />
                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
  ...
  <h4 className="text-sm font-medium text-gray-700 mb-3">Product Location</h4>
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
    {[
      ...
      <div key={key}>
        <label className="block text-sm font-medium text-gray-700 mb-1">
          {label} {key !== "location" && <span className="text-red-500">*</span>}
        </label>
        <div className="relative">
          <input
            className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-green-500"
...
>
  <div className="w-6 h-6 bg-green-500 rounded-full flex items-center justify-center">
    <FaUndo className="text-white text-xs" />
              ...
              <button
                onClick={onClose}
                className="border border-red-500 text-red-500 px-4 py-2 rounded text-sm font-medium hover:bg-red-50"
              >
                Cancel
              </button>
              <button 
  className={`px-6 py-2.5 rounded-md text-sm font-medium text-white shadow-sm transition-all 
